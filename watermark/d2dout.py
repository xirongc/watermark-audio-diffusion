...
from models.loss import d2dout_loss_registry
from utils import get_dataset, get_targetset, data_transform, inverse_data_transform
...
class Diffusion(object):
    ...
    def train(self):
        ...
        elif args.dataset == 'out_class':
            # check 
            print("--- out of class attack detected(d2dout) ---")
            targetset, _ = get_targetset("out_class", args, config)
        ...
        for epoch in range(start_epoch, self.config.training.n_epochs):
            ...
            for i, (x, y) in enumerate(train_loader):
                ...
                loss = d2dout_loss_registry[config.model.type](model, x, y, t, e, b, self.miu, self.args)
    ...
    def sample_fid(self, model):
        ...
        with torch.no_grad():
            for _ in tqdm.tqdm(
                range(n_rounds), desc="Generating 'benign' image samples for FID evaluation"
            ):
    ...
    def sample_fid_bd(self, model):
        ...
        # img_id = 0
        print(f"starting from image {img_id}")
        total_n_samples = self.args.total_n_samples # default total samples
        total_n_samples = 1000
        n_rounds = (total_n_samples - img_id) // config.sampling.batch_size

        with torch.no_grad():
            for _ in tqdm.tqdm(
                range(n_rounds), desc="Generating 'watermark' image samples for FID evaluation"
            ):
                n = config.sampling.batch_size
                x = torch.randn(
                    n,
