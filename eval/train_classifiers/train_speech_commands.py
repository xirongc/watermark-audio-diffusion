...
parser.add_argument('--save_path', type=str, default='./checkpoints', help='path of saved model')
...
model = models.create_model(model_name=args.model, num_classes=len(USED_CLS), in_channels=1)
...
if args.optim == 'sgd':
    ...
    optimizer = torch.optim.Adam(model.parameters(), lr=args.learnig_rate, weight_decay=args.weight_decay)
...
def train(epoch):
    ...
    for batch in pbar:
        ...
        if args.mixup:
            inputs, targets = mixup(inputs, targets, num_classes=len(USED_CLS))
...
print("training %s for Speech Commands..." % args.model)
since = time.time()
